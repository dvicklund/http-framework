...
var Logger = module.exports = exports = {
  ...
  logReq: function(req) {
    ...
    writeFile(logFolder, makeReqLog(req), "Request");
  },

  logRes: function(res) {
    var logFolder = __dirname + '/log/responses/';
    writeFile(logFolder, makeResLog(res), "Response");
  },
...
function writeFile(logFolder, data, type) {
  var date = new Date();
  ...
  fs.access(logFolder, fs.F_OK, function(err) {
    if(err) {
      ...
      fs.writeFile(logFolder + dateString + '.log', data, function(err) {
        err ? console.log("Error - can't write file!") : console.log(type + ' log saved!');
      });
    } else { 
      fs.writeFile(logFolder + dateString + '.log', data, function(err) {
        err ? console.log("Error - can't write file!") : console.log(type + ' log saved!');
